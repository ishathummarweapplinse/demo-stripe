...
import { deeltefetchuser, fetchuser, usreblock } from '../../../services/action/user';
...
const BasicBadges = () => {
  ...
  // const userdataloading = useSelector((state) => state.userreducer?.loading);
  ...
  useEffect(() => {
    if (userdatainfo?.length > 0) {
      localStorage.setItem('userlist', JSON.stringify(userdatainfo));
    }
  }, [userdatainfo]);
  ...
  useEffect(() => {
    ...
    setuserdata(JSON.parse(data));
    dispatch(
      fetchuser(currentPage, itemsPerPage, (total) => {
        setTotalPage(total);
      })
    );
  ...
  const handleSearch = (value) => {
    ...
    const searchData = userdatainfo
      .filter((item) => {
        const name = item.name?.toLowerCase().includes(searchTerm);
        const email = item.email_address.toLowerCase().includes(searchTerm);
        return name || email;
      })
      .map((item) => ({
        key: item._id,
